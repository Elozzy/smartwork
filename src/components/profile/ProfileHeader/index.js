...
const ProfileHeader = ({ authUser }) => {
  const {tier, name, passport} = authUser;
  const accType = ( tier === '1' ) ? "Unverified Account" :  (tier === '2' ) ? "Classic Account" : "Platinium Account";
  return (
        ...
        <div className="gx-profile-banner-top">
          <div className="gx-profile-banner-top-left">
            <div className="gx-profile-banner-avatar">
              <Avatar className="gx-size-90" alt="..." src={passport}/>
            </div>
            <div className="gx-profile-banner-avatar-info text-center">
              <h2 className="gx-mb-2 gx-mb-sm-3 gx-fs-xxl gx-font-weight-light">{name}</h2>
              <p className="gx-mb-0 gx-fs-lg">{accType}</p>
            </div>
          </div>
